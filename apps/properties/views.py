...
def properties_list_view(request):
    ...
    properties = Property.published.all().select_related('user').prefetch_related('images')
...
def property_detail_view(request, pk):
    ...
    try:
        property_obj = Property.objects.select_related('user').prefetch_related('images').get(pk=pk)
            ...
            try:
                with transaction.atomic():
                    ...
                    if is_htmx:
                        ...
                        response['HX-Redirect'] = reverse('properties:detail', args=[property_obj.pk])
                        return response
                    else:
