# Documentation Index

Complete index of all Property Hub documentation.

## 📖 Main Documentation

- [Documentation Home](./README.md) - Start here
- [Documentation Index](./INDEX.md) - This file

## 🏗 Architecture

- [Architecture Overview](./architecture/overview.md) - System architecture and design
- [Frontend Architecture](./architecture/frontend.md) - Django Tailwind CLI, Cotton components, and static assets

## 🚀 Development

- [Frontend Setup](./development/frontend-setup.md) - Tailwind, Cotton, and browser JavaScript workflow
- [Chat Client](./development/chat-client.md) - WebSocket chat client documentation

## 📦 Deployment

- [Deployment Checklist](./deployment/checklist.md) - Production deployment guide

## 📚 Guides

- [Contributing Guide](./guides/contributing.md) - How to contribute to the project

*More guides coming soon - step-by-step tutorials for common tasks*

## Quick Reference

### For New Developers
1. Read [Architecture Overview](./architecture/overview.md)
2. Follow setup in [main README](../README.md)
3. Review [Frontend Setup](./development/frontend-setup.md)
4. Check [Contributing Guide](./guides/contributing.md)

### For UI Developers
1. [Frontend Architecture](./architecture/frontend.md)
2. [Frontend Setup Guide](./development/frontend-setup.md)
3. [Chat Client Docs](./development/chat-client.md)

### For DevOps Engineers
1. [Deployment Checklist](./deployment/checklist.md)
2. [Architecture Overview](./architecture/overview.md)
3. [Frontend Architecture](./architecture/frontend.md) (for build process)

### For Contributors
1. [Contributing Guide](./guides/contributing.md)
2. [Architecture Overview](./architecture/overview.md)
3. Relevant development docs

## Documentation by Topic

### Static Files & Assets
- [Frontend Architecture](./architecture/frontend.md) - Complete UI pipeline guide
- [Frontend Setup](./development/frontend-setup.md) - Development workflow

### Real-time Features
- [Chat Client](./development/chat-client.md) - WebSocket implementation
- [Architecture Overview](./architecture/overview.md) - WebSocket flow

### Deployment & Production
- [Deployment Checklist](./deployment/checklist.md) - Complete checklist
- [Frontend Architecture](./architecture/frontend.md) - Build process

### Contributing
- [Contributing Guide](./guides/contributing.md) - Complete guide
- [Architecture Overview](./architecture/overview.md) - Understanding the system

## External Resources

### Django
- [Django Documentation](https://docs.djangoproject.com/)
- [Django Deployment Checklist](https://docs.djangoproject.com/en/stable/howto/deployment/checklist/)

### Frontend
- [Tailwind CSS](https://tailwindcss.com/docs)
- [DaisyUI](https://daisyui.com/)
- [HTMX](https://htmx.org/docs/)
- [Alpine.js](https://alpinejs.dev/)

### Infrastructure
- [Docker Documentation](https://docs.docker.com/)
- [WhiteNoise](http://whitenoise.evans.io/)
- [Django Channels](https://channels.readthedocs.io/)

## Need Help?

1. **Search this documentation** - Use your editor's search or grep
2. **Check the main README** - [../README.md](../README.md)
3. **Review inline code comments** - Check the source code
4. **Ask the team** - Open a GitHub Discussion
5. **Report missing docs** - Open an issue

## Contributing to Documentation

See the [Contributing Guide](./guides/contributing.md) for:
- Documentation standards
- Where to add new docs
- How to update existing docs
- Documentation review process

---

**Last Updated:** February 2026
**Maintainers:** Property Hub Team
